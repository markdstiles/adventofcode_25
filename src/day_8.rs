...
pub fn do_part2() -> anyhow::Result<i64> {
    ...
    if let Some(last_pair) = get_last_pair(&pairs, input.len()) {
        result = last_pair.first.x * last_pair.second.x;
    }
...
fn get_last_pair(pairs: &Vec<JunctionPair>, junction_count: usize) -> Option<JunctionPair> {
    let mut circuits: Vec<Vec<Position>> = vec![];
    let mut position_circuit_index: HashMap<Position, usize> = HashMap::new();
    let mut last_pair: Option<JunctionPair> = None;

    for pair in pairs {

        if circuits.iter().any(|c| c.len() == junction_count) {
            return last_pair
        }
...
fn part2_proof() {
    ...
    let last_pair = get_last_pair(&pairs, input.len());

    assert_ne!(last_pair, None);
    if let Some(pair) = last_pair {
        assert_eq!(pair.first.x, 216);
        assert_eq!(pair.second.x, 117);
        assert_eq!(pair.first.x * pair.second.x, 25272);
    }
}